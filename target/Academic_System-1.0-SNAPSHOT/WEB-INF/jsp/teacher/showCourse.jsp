				...
				<div class="panel-footer">
					<c:if test="${page != null}">
						<nav style="text-align: center">
							<ul class="pagination">
								<li><a href="/teacher/showCourse?page=${page.upPageNo}">&laquo;上一页</a></li>
								<li class="active"><a href="/teacher/showCourse?page=${page.currentPageNo}">${page.currentPageNo}</a></li>
								<c:if test="${page.currentPageNo+1 <= page.totalCount}">
									<li>
										<a href="/teacher/showCourse?page=${page.currentPageNo+1}">${page.currentPageNo+1}</a>
									</li>
								</c:if>
								<c:if test="${page.currentPageNo+2 <= page.totalCount}">
									<li>
										<a href="/teacher/showCourse?page=${page.currentPageNo+2}">${page.currentPageNo+2}</a>
									</li>
								</c:if>
								<c:if test="${page.currentPageNo+3 <= page.totalCount}">
									<li>
										<a href="/teacher/showCourse?page=${page.currentPageNo+3}">${page.currentPageNo+3}</a>
									</li>
								</c:if>
								<c:if test="${page.currentPageNo+4 <= page.totalCount}">
									<li>
										<a href="/teacher/showCourse?page=${page.currentPageNo+4}">${page.currentPageNo+4}</a>
									</li>
								</c:if>
								<li><a href="/teacher/showCourse?page=${page.totalCount}">最后一页&raquo;</a></li>
							</ul>
						</nav>
					</c:if>
				</div>
